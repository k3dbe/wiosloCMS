...
abstract class BaseUserRoleQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'wiosloCMS\\UserBundle\\Model\\UserRole';
        }
    ...
    {
        $sql = 'SELECT `user_id`, `role_id` FROM `UserRole` WHERE `user_id` = :p0 AND `role_id` = :p1';
        try {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \wiosloCMS\UserBundle\Model\RoleQuery A secondary query class using the current class as primary query
     */
    public function useRoleQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinRole($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Role', '\wiosloCMS\UserBundle\Model\RoleQuery');
    }
