...
use wiosloCMS\UserBundle\Model\map\UserRoleTableMap;
...
abstract class BaseUserRolePeer
{
    ...
    const OM_CLASS = 'wiosloCMS\\UserBundle\\Model\\UserRole';

    /** the related TableMap class for this table */
    const TM_CLASS = 'wiosloCMS\\UserBundle\\Model\\map\\UserRoleTableMap';
    ...
    /**
     * Method to invalidate the instance pool of all tables related to UserRole
    ...
    /**
     * Selects a collection of UserRole objects pre-filled with all related objects except User.
     *
     * @param      Criteria  $criteria
     * @param      PropelPDO $con
     * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
    ...
    public static function buildTableMap()
    {
      ...
      if (!$dbMap->hasTable(BaseUserRolePeer::TABLE_NAME)) {
        $dbMap->addTableObject(new \wiosloCMS\UserBundle\Model\map\UserRoleTableMap());
      }
    }
    ...
    /**
     * Deletes all rows from the UserRole table.
     *
     * @param      PropelPDO $con the connection to use
     * @return int             The number of affected rows (if supported by underlying database driver).
