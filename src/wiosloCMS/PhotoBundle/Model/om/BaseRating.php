...
abstract class BaseRating extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'wiosloCMS\\PhotoBundle\\Model\\RatingPeer';
    ...
    public function setPhotoId($v)
    {
        ...
        if ($this->photo_id !== $v) {
            $this->photo_id = $v;
            $this->modifiedColumns[] = RatingPeer::PHOTO_ID;
        }
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = RatingQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        $sql = sprintf(
            'INSERT INTO `PhotoRating` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Rating'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Rating'][$this->getPrimaryKey()] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aPhoto) {
                $result['Photo'] = $this->aPhoto->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
    ...
    {
        $criteria = new Criteria(RatingPeer::DATABASE_NAME);
        $criteria->add(RatingPeer::PHOTO_ID, $this->photo_id);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            $relObj = $this->getPhoto();
            if ($relObj) {
                $copyObj->setPhoto($relObj->copy($deepCopy));
            }
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setPhotoId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     * @throws PropelException
     */
    public function getPhoto(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aPhoto === null && ($this->photo_id !== null) && $doQuery) {
            $this->aPhoto = PhotoQuery::create()->findPk($this->photo_id, $con);
            // Because this foreign key represents a one-to-one relationship, we will create a bi-directional association.
            $this->aPhoto->setRating($this);
        }
     ...
     * If this Rating is new, it will return
    ...
    public function setUserRates(PropelCollection $userRates, PropelPDO $con = null)
    {
        ...
        foreach ($userRatesToDelete as $userRateRemoved) {
            $userRateRemoved->setRating(null);
        }
     ...
     * Otherwise if this Rating is new, it will return
     * an empty collection; or if this Rating has previously
     * been saved, it will retrieve related UserRates from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Rating.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     ...
     * @return PropelObjectCollection|UserRate[] List of UserRate objects
     */
    public function getUserRatesJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = UserRateQuery::create(null, $criteria);
     ...
     *
     * @param  User $user The UserRate object to relate
     * @return Rating The current object (for fluent API support)
     */
    ...
    protected function doAddUser(User $user)
    {
        ...
        if (!$user->getRatings()->contains($this)) {
            ...
            $foreignCollection = $user->getRatings();
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->aPhoto instanceof Persistent) {
              $this->aPhoto->clearAllReferences($deep);
            }
    ...
    {
        return (string) $this->exportTo(RatingPeer::DEFAULT_STRING_FORMAT);
    }
